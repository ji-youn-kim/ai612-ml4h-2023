...
@register_dataset("20238037_pretrain_dataset")
class Pretrain20238037(BaseDataset):
    """
    TODO:
    ...
    def __getitem__(self, index):
        ...
        if len(item['input']) > self.max_event_size:
            item['input'] = item['input'][:self.max_event_size]
        half = int(len(item['input']) / 2)
        # need to have atleast 1 event
        item['input'] = np.array([item['input'][:half], item['input'][half:]], dtype=object)
    ...
    def collator(self, samples):
        ...
        if len(samples) == 0:
            return {}
        input = dict()
        input['input'] = [j.astype(np.int16) for s in samples for j in s['input']] # Make positive pair adjacent (B -> 2*B)
